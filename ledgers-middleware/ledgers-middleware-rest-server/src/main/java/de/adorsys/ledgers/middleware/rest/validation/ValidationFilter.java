    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (servletRequest.getContentLength() > 0) {
            try {
                ...
                Optional<JsonNode> invalidICurrency = validate(readValuesByField(servletRequest, CURRENCY), n -> currencyService.getSupportedCurrencies().toString().contains(n.asText()));
                if (invalidICurrency.isPresent()) {
                    log.error("Invalid currency: {}", invalidICurrency.get().asText());
                    ((HttpServletResponse) response).sendError(400, String.format("Invalid currency %s", invalidICurrency.get().asText()));
                    return;
                }
            } catch (IOException e) {
